...
const Estimatesdetails = ({ route, navigation }: EstimatesdetailsScreenProps) => {
  ...
  const dropdownAnimRef = useRef<Animated.CompositeAnimation | null>(null);
  ...
  const toggleDropdown = () => {
    ...
    dropdownAnimRef.current?.stop();

    setAnimating(true);
    if (!visible) {
      setVisible(true); // set first to mount
      const anim = Animated.timing(dropdownAnim, {
        toValue: 1,
        duration: 250,
      ...
      dropdownAnimRef.current = anim;
      anim.start(() => setAnimating(false));
    } else {
      const anim = Animated.timing(dropdownAnim, {
        toValue: 0,
        duration: 200,

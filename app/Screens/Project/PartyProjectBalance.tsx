...
const PartyProjectBalance = ({ route, navigation }: PartyProjectBalanceScreenProps) => {
    ...
    const tabAnimRef = useRef<Animated.CompositeAnimation | null>(null);


    const handleTabChange = (tab: 'received' | 'salary') => {
        // stop any running animation before starting a new one
        tabAnimRef.current?.stop();

        const fadeOutAndDown = Animated.parallel([
        ...
        tabAnimRef.current = fadeOutAndDown;

        fadeOutAndDown.start(() => {
            setActiveTab(tab);

            const fadeInAndUp = Animated.parallel([
                Animated.timing(fadeAnim, {
                    toValue: 1,
                    duration: 200,
                    useNativeDriver: true,
            ...
            tabAnimRef.current = fadeInAndUp;
            fadeInAndUp.start();
        });
    };

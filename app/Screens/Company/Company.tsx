...
const Company = ({ route }: CompanyScreenProps) => {
 ...
 let completedQtyInitial = 0;
 let todayWorkQtyInitial = 0;
 const today = new Date();

 statusHistory.forEach((entry) => {
 const qty = safeNumber(entry.todays_progress);
 if (!qty) return;
 completedQtyInitial += qty;
 if (isSameDay(entry.date || entry.createdAt, today)) {
 todayWorkQtyInitial += qty;
 }
 });

 let totalWorkQtyInitial: number | undefined;
 let pendingQtyInitial: number | undefined;

 if (typeof percentCompleteRaw === "number" && percentCompleteRaw > 0) {
 // total = completed / (percentComplete / 100)
 totalWorkQtyInitial = completedQtyInitial / (percentCompleteRaw / 100);
 pendingQtyInitial = totalWorkQtyInitial - completedQtyInitial;
 }

 const initialPercentSafe = Number(
 Math.min(Math.max(percentCompleteRaw ?? 0, 0), 100).toFixed(1)
 );

 // 🔹 Local state so UI auto-updates after status/qty change
 const [completedQty, setCompletedQty] =
 useState<number>(completedQtyInitial);
 const [todayWorkQty, setTodayWorkQty] =
 useState<number>(todayWorkQtyInitial);
 const [totalWorkQty] = useState<number | undefined>(totalWorkQtyInitial);
 const [pendingQty, setPendingQty] =
 useState<number | undefined>(pendingQtyInitial);
 const [percentSafe, setPercentSafe] =
 useState<number>(initialPercentSafe);
 ...
 // 🔹 Update local progress numbers so UI refreshes instantly
 if (effectiveQty !== null && effectiveQty > 0) {
 setCompletedQty((prevCompleted) => {
 const newCompleted = prevCompleted + effectiveQty;

 // update today's work
 setTodayWorkQty((prevToday) => prevToday + effectiveQty);

 // update pending qty based on totalWorkQty (if known)
 setPendingQty((prevPending) => {
 if (typeof totalWorkQty === "number" && totalWorkQty > 0) {
 return Math.max(totalWorkQty - newCompleted, 0);
 }
 return prevPending;
 });

 // update percentSafe from new completed vs total
 setPercentSafe((prevPercent) => {
 if (typeof totalWorkQty === "number" && totalWorkQty > 0) {
 const raw = (newCompleted / totalWorkQty) * 100;
 return Number(
 Math.min(Math.max(raw, 0), 100).toFixed(1)
 );
 }
 return prevPercent;
 });
